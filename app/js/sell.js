...
const URL_SALE_CREDIT_API =
    "http://localhost/Abarrotes/api/AllSales_Credit.php";
...
class Product {
    ...
    addToTable() {
        ...
        TB.insertAdjacentHTML(
            "afterbegin",
            `<tr id='tr${this.code}' data-code='${this.code}'>
                <td>${this.code}</td>
                <td>${this.name}</td>
        ...
        IMG_DEL.addEventListener("click", () => {
            TB.removeChild(TR);
            this.removeFromCar(TR.dataset.code);
            this.updateTotal();
        });
        //CHANGE IMG TO RED
    ...
    addCancelListener() {
        ...
        btnCancel.addEventListener("click", () => {
            swal({
                ...
                text:
                    "La compra se cancelara completamente,esta seguro que quiere cancelar ?",
                icon: "warning",
                color: "#123",
                buttons: ["No, cancelar", "  Si  "],
                cancel: "cancelar",
            }).then((result) => {
                if (result) {
                    //cancelar la transaccion
                    swal("Cancelada!", "Compra Cancelada", "success");
    ...
    checkChange() {
        ...
        let label_change_missing = document.getElementById(
            ID_LABEL_CHANGE_MISSING
        );
        ...
        if (event.keyCode == "13") {
            if (input.value >= this.total) {
                ...
                setTimeout(() => {
                    btnFinishSale.addEventListener(
                        "click",
                        this.finishedSell.bind(this)
                    );
                    btnFinishSale.focus();
                }, 10);
            } else {
    ...
    updateCart() {
        ...
        let totalQuantity =
            parseFloat(this.car[indexFound].quantity) +
            parseFloat(this.quantity);
        let subtotal =
            parseFloat(this.car[indexFound].subtotal) +
    ...
    async finishedSell() {
        try {
            ...
            const request = await fetch(URL_SALES_API, {
                ...
                body: `total=${this.total}&products=${JSON.stringify(
                    this.car
                )}&client=${this.client}`,
            });
            ...
            if (response.status == 200) {
                ...
                swal({
                    ...
                    footer:
                        "<a href>Ingresa algun producto para poder continuar</a>",
                });
            }
        } catch (err) {
            console.log(err);
        ...
        setTimeout(async () => {
            ...
            json.forEach((client) => {
                select.insertAdjacentHTML(
                    "afterbegin",
                    `<option value=${client.number} data-limit=${client.limit} data-used=${client.total_used} >
                    ${client.name}</option>`
                );
            });
        }, 200);
    ...
    openPopUpSellCredit() {
        ...
        btnFinishCredit.addEventListener("click", () => {
            ...
            let moneyfree =
                parseFloat(select[index].dataset.limit) -
                parseFloat(select[index].dataset.used);
    ...
    addProductToTableSearch(product) {
        //variables
        let tb_search = document.querySelector(
            `#${ID_TABLEBODY_SEARCH_PRODUCT}`
        );
